...
def fit(network,X_train_raw_data,y_train_raw_data,X_val,y_val,lr,start_time,verbose=False,earlystop=None,number_of_batches=None):
    sum_time = 0
    loss_history = []
    accuracy_history = []
    val_loss_history = []
...
def train_validation_split(X,y,validation_ratio=0.15):
    ...
    return list(X_shuffled[right_border:]),list(X_shuffled[0:right_border]),list(y_shuffled[right_border:]),list(y_shuffled[0:right_border])
...
if __name__ == "__main__":
    ...
    #y_test_file_path = "E:\\PyCharmProjects\\HW3\\data\\test\\test_image.csv"
    ...
    y_test_labels_file_path = "test_label.csv"

    #X_train_file_path = "batch_for_augmentation.csv"
    #y_train_file_path = "label_for_augmentation.csv"
    ...
    X_train_data,X_validation_data,y_train_data,y_validation_data = train_validation_split(X_train_raw_data,y_train_raw_data,
    ...
    print('Full time: {}'.format((time.time() - start_t)/60))
